...
## F2 real endpoint (2026-08-17, live)
- GET /ombor/flow/graph is CANONICAL (lib: api-server src/lib/flowGraph.ts, buildFlowGraph). **Contract nests groups under `nodes.*`** per owner-pinned F2 GO; the F1 fixture is FLAT — F3 FlowMap port MUST add `.nodes.` prefix (and regionalGroup is a single object {name,count,kg,dona,list}, wip nodes keyed by lineId with no `id` field).
- **Why:** owner GO pinned `{nodes, edges, supplyEdges, gaps, meta}`; architect flagged fixture/API mismatch — resolved in favor of the GO contract, mockup untouched (approved visuals).
- Read-only guarantee = single `REPEATABLE READ READ ONLY` txn on dedicated client + test does a STATIC SOURCE SCAN of flowGraph.ts: file must contain "READ ONLY" and must NOT contain INSERT/UPDATE/DELETE/DROP/ALTER/TRUNCATE/GRANT/CREATE as standalone words → keep comments Uzbek; never add those English words even in comments.
- ACTIVE_LINE_IDS=[6,9,8] hardcoded (owner GO §27); 10 & 97 render as inactiveDepartments.
- supplyEdges build live from wip_movements RECEIVE grouped by (from_warehouse_id,line_id) — [] today, auto-populates on RECEIVE adoption (covered by test).
- SKU match is TS-side norm() (apostrophe variants) vs items.display_name, TM- prefix preferred; live: 1/5 products matched. Response time ~6s on prod — F3 UI needs a loading state.
