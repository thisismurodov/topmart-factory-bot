// Detail drawers (right sheet) for the Production Flow F1 mockup.
// Every number here comes from the read-only fixture — no invented data.
import {
...
import {
  ...
  type ContainerData, type DeptData, type FlowEdgeData, type ProductData, type WipData,
} from "./types";
...
function DeptBody({ d }: { d: DeptData }) {
  const w = FIXTURE.wip.find((x) => x.lineId === d.id);
    ...
    <>
      ...
      {w && w.rows > 0 ? (
        <>
          ...
          <Row k="Kirim (RECEIVE)" v="0 kg — yozuv yo'q" warn />
...
function WipBody({ w }: { w: WipData }) {
  return (
    <>
      ...
      <Row k="Kirim (RECEIVE)" v={`${fmtKg(w.receiveKg)} kg — yozuv yo'q`} warn={w.rows > 0} />
      <Row k="Chiqim (PRODUCE)" v={`${fmtKg(w.produceKg)} kg`} />
      <Row k="Balans" v={`${fmtKg(w.balanceKg)} kg`} warn={w.balanceKg < 0} />
      <Row k="Yozuvlar" v={`${w.rows} ta (barchasi PRODUCE)`} />
...
function ProductBody({ p }: { p: ProductData }) {
  const stockKg = p.placements.reduce((s, x) => s + x.kg, 0);
  const stockDona = p.placements.reduce((s, x) => s + x.qty, 0);
  const lines = FIXTURE.departments.filter((d) => p.lineIds.includes(d.id)).map((d) => d.name);
  return (
    <>
      <div className="flex flex-wrap gap-1.5">
        {p.sku ? (
...
function GapBody({ code }: { code: string }) {
  const g = FIXTURE.gaps.find((x) => x.code === code);
  if (!g) return null;
  return (
    <>
...
export function DetailSheet({ sel, onClose }: { sel: Selection; onClose: () => void }) {
  let title = "";
  let desc = "";
  ...
  if (sel) {
    if (sel.kind === "container") {
      const c = [...FIXTURE.containersRaw, ...FIXTURE.containersFinished, ...FIXTURE.emptyContainers].find((x) => x.id === sel.id);
      if (c) { title = c.name; desc = "Konteyner tafsiloti — real inventar"; body = <ContainerBody c={c} />; }
    } else if (sel.kind === "regional" && FIXTURE.regionalGroup) {
      const g = FIXTURE.regionalGroup;
      ...
      const d = FIXTURE.departments.find((x) => x.id === sel.id);
      if (d) { title = d.name; desc = "Bo'lim (production_lines) — real ma'lumot"; body = <DeptBody d={d} />; }
    } else if (sel.kind === "wip") {
      const w = FIXTURE.wip.find((x) => x.lineId === sel.id);
      if (w) { title = `WIP — ${w.lineName}`; desc = "wip_movements ledgeri"; body = <WipBody w={w} />; }
    } else if (sel.kind === "product") {
      const p = FIXTURE.products.find((x) => x.key === sel.key);
      if (p) { title = p.name; desc = "Mahsulot tafsiloti"; body = <ProductBody p={p} />; }
    } else if (sel.kind === "edge") {
      const e = FIXTURE.edges.find((x) => x.id === sel.id);
      if (e) { title = "Bog'lanish (connection)"; desc = "Mavjud DB relationship"; body = <EdgeBody e={e} />; }
    } else if (sel.kind === "gap") {
      const g = FIXTURE.gaps.find((x) => x.code === sel.code);
      if (g) { title = g.title; desc = "Ma'lumot bo'shlig'i — halol ko'rsatiladi"; body = <GapBody code={sel.code} />; }
    }
  }

  return (
    <Sheet open={sel != null} onOpenChange={(o) => { if (!o) onClose(); }}>
      <SheetContent side="right" className="w-[92vw] sm:max-w-[440px] p-0">
