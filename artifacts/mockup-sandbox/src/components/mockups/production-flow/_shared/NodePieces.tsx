// Custom React Flow node components for the Production Flow F1 mockup.
...
export function ContainerNode({ data }: any) {
  ...
  return (
    <div className={`${card} ${dim} ${c.derived === "empty" ? "border-dashed" : ""}`}>
      <H />
...
export function RegionalNode({ data }: any) {
  const g = data.group;
  const dim = data.dimmed ? "opacity-25" : "";
  return (
    <div className={`${card} ${dim} bg-zinc-50`}>
      <H />
      <div className="flex items-center gap-2">
...
export function DeptNode({ data }: any) {
  const d: DeptData = data.dept;
  const dim = data.dimmed ? "opacity-25" : "";
  const workerCount = d.workers.length;
  return (
    <div className={`${card} ${dim} border-indigo-200 bg-indigo-50/60`}>
      <H />
      <div className="flex items-center gap-2">
...
export function WipNode({ data }: any) {
      ...
      {noLedger ? (
        ...
        <>
          ...
          <div className="mt-1.5 text-[11px] text-zinc-600 tabular-nums">
            RECEIVE: 0 kg · PRODUCE: {fmtKg(w.produceKg)} kg · {w.rows} yozuv
          </div>
        </>
      )}
    </div>
...
export function ProductNode({ data }: any) {
...
export function GapNode({ data }: any) {
  return (
    <div className="w-[150px] rounded-lg border-2 border-dashed border-orange-300 bg-orange-50/90 px-2.5 py-2 text-center">
      <H />
