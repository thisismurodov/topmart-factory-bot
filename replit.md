...
- AI Route Planning: `artifacts/api-server/src/lib/routePlanner.ts` — balanced k-means klasterlash + nearest-neighbor + 2-opt tartiblash; `splitOutliers` mediandan 60km+ uzoq GPS'li do'konlarni chiqarib tashlaydi (`badCoord[]` sifatida qaytadi). `POST /api/distribution/route-plan` (auth wall ortida): `{viloyat, agentId, dryRun?, replace?}` — dryRun=preview, replace'siz mavjud marshrut bo'lsa 409. Saqlash tranzaksiyada (DELETE+INSERT). Limit: maks 6 kun × ~28 do'kon (~168 do'kon/viloyat) — kattaroq viloyat uchun kunlik limit oshadi. `route-map` endpoint endi `routeStats[]` (km, ~vaqt, ⭐score, boshlanish/tugash) va `allAgents[]` ham qaytaradi. UI: `RoutePlanDialog.tsx` (Marshrut tabidagi "AI marshrut" tugmasi).
